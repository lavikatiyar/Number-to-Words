...
def get_words(number, index):
  if number == '0': return 'Zero'

  #The zfill() method adds zeros (0) at thebeginning of the string, until it reaches the specified length.
...
def convert_to_words(number):
  ...
  for i in range(length - 1, -1, -3):
    words.append(
      get_words(str(number)[0 if i - 2 < 0 else i - 2:i + 1], copy - count))
